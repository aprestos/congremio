<script setup lang="ts">
import { defineProps } from "vue";
import type { LibraryGame } from "@/features/library/game.model.ts";

const props = defineProps<{ games: LibraryGame[] }>();
</script>

<template>
  <div class="mx-auto max-w-2xl lg:max-w-7xl">
    <div
      class="grid grid-cols-1 gap-y-12 sm:grid-cols-2 sm:gap-x-6 lg:grid-cols-4 xl:gap-x-8"
    >
      <div v-for="game in props.games" :key="game.id">
        <div class="relative">
          <div class="relative h-72 w-full overflow-hidden rounded-lg">
            <img
              :src="game.game.image"
              :alt="game.game.name + ' cover image'"
              class="size-full object-cover"
            />
          </div>
          <div class="relative mt-4">
            <h3 class="text-sm font-medium text-gray-900">
              {{ game.game.name }}
            </h3>
            <p class="mt-1 text-sm text-gray-500">{{ game.game.year }}</p>
          </div>
          <div
            v-if="game.status && game.status !== 'available'"
            class="absolute inset-x-0 top-0 flex h-72 items-end justify-end overflow-hidden rounded-lg p-4"
          >
            <div
              v-if="game.status === 'not-available'"
              aria-hidden="true"
              class="absolute inset-x-0 bottom-0 h-36 bg-linear-to-t from-red-700 opacity-100"
            />
            <div
              v-if="game.status === 'withdrawn'"
              aria-hidden="true"
              class="absolute inset-x-0 bottom-0 h-36 bg-linear-to-t from-yellow-700 opacity-100"
            />

            <p
              class="relative text-lg text-shadow-lg font-semibold text-gray-50"
            >
              {{
                game.status === "not-available" ? "Not available" : "Withdrawn"
              }}
            </p>
          </div>
        </div>
        <!--        <div class="mt-6">-->
        <!--          <a-->
        <!--            :href="#"-->
        <!--            class="relative flex items-center justify-center rounded-md border border-transparent bg-gray-100 px-8 py-2 text-sm font-medium text-gray-900 hover:bg-gray-200"-->
        <!--            >Add to bag<span class="sr-only">, {{ game.name }}</span></a-->
        <!--          >-->
        <!--        </div>-->
      </div>
    </div>
  </div>
</template>

<style scoped></style>
